...
class Mage_Core_Model_Email_Template extends Mage_Core_Model_Abstract {
     ...
     * @return  boolean
     **/
    public function send($email, $name = null, array $variables = array(),$attach = false) {
        ...
        if($attach) {
            unlink(Mage::getBaseDir('export')."/DealList.csv");
        }
        return true;
    }
     ...
     * @return  Mage_Core_Model_Email_Template
     */
    public function sendTransactional($templateId, $sender, $email, $name, $vars=array(), $storeId=null,$sendAttachment = false) {
        ...
        if($sendAttachment) {
            $this->setSentSuccess($this->send($email, $name, $vars,true));
        }
        else {
