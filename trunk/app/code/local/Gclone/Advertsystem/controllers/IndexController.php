            ...
            try {
                if (!$session->isLoggedIn()) {
                    ...
                    if ($fetchInvite[0]['count'] == 0) {
                        $inviteQuery = $read->query("insert into " . $inviteTable . " values ('',$customerId,'','$friendName','$friendEmail','',0)");
    ...
    public function importAction() {
        ...
        if (count($inviteRecords) > 0) {
            foreach ($inviteRecords as $key => $record) {
                ...
                $query = "insert into $inviteExport (invite_id,customer_id,friend_id,friend_name,friend_email,created_at,expired) values ('',$refererId,$referralId,'$frndName','$frndEmail','$createdAt',0)";
                $exportRecords = $read->query($query);
            }
        }
        ...
        if (count($discountRecords) > 0) {
            foreach ($discountRecords as $key => $record) {
                ...
                $query = "insert into $discountExport (discount_id,customer_id,amount,used,remaining) values ('',$refererId,'$amount',0,0)";
                $exportRecords = $read->query($query);
            }
        }
    }
