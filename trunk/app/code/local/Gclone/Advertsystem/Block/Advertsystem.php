 ...
 */
class Gclone_Advertsystem_Block_Advertsystem extends Mage_Core_Block_Template {

    public function _prepareLayout() {
        return parent::_prepareLayout();
    }

    public function getAdvertsystem() {
        if (!$this->hasData('advertsystem')) {
            $this->setData('advertsystem', Mage::registry('advertsystem'));
        }
        return $this->getData('advertsystem');
    }
    ...
    public function getRuleDiscount() {
        ...
        $select = $read->select()
                        ->from(array('ct' => $ruleTable), array('ct.discount_amount'));
        $rule = $read->fetchAll($select);
        return $rule[0]['discount_amount'];
    }
    ...
    public function getPercent() {
        ...
        $customerId = $session->getId();
        $select = $read->select()
                        ->from(array('ct' => $discountTable), array('ct.percent'))
                        ->where('ct.customer_id = (?)', $customerId);
        $percent = $read->fetchOne($select);
