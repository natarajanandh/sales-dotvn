...
class Gclone_Couponvalidator_Block_Adminhtml_Couponvalidator_Griding extends Mage_Adminhtml_Block_Widget_Griding
{
    ...
    public function __construct()
    {
        ...
        $this->setTemplate('report/griding.phtml');
...
//                );
    ...
    public function getdealdetails()
    {
        ...
        if(isset($update)){
            ...
            $statusUpdate = $couponUpdate->query("update ".$tprefix."ordercustomer set status = '2', updated_on=now() where ordercustomer_id = '$update'" );
...
//            $orders_list = $read->fetchAll($select);
            ...
            //$couponTable = $resource->getTableName('ordercustomer/ordercustomer');
            $tprefix = (string)Mage::getConfig()->getTablePrefix();
            $couponTable = $tprefix.'ordercustomer';
            
            $select = $read->select()
            ->from(array('cp'=>$orderTable))
            ->join(array('pei'=>$couponTable), 'pei.order_id=cp.entity_id',array('ordercustomer_id','uniqueid','customer_name','product_name', 'quantity','quantitynumber','created_time','status', 'updated_on'))
...
//            }
    ...
    {
        return $this->__('No records found for this Coupon Code.');
    }
