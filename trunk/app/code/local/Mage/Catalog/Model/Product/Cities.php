 ...
 */
class Mage_Catalog_Model_Product_Cities extends Mage_Core_Model_Abstract {
    ...
    protected function _construct() {
        $this->_init('catalog/product_cities');
    }
    ...
    public function getVisibleStatusIds() {
        return array(self::DEALCITIES_ENABLED);
    }
    ...
    static public function getOptionArray() {
        ...
        $flatorderTable = $tprefix."catalog_product_entity_varchar";
        $read = $resource->getConnection('read');
        $select = $read->select()
        ->from(array('cp'=>$flatorderTable),array('value'))
        ->where('attribute_id= 562');
        $list = $read->fetchAll($select);
        $city = array();
        $attribute = Mage::getModel('eav/config')->getAttribute('catalog_product', '562'); // get the cities attribute id 548


        foreach($list as $orders_list){
            $list= explode(",",$orders_list['value']);
            $citylabel = array();
            for($i=0;$i<count($list);$i++){
                foreach ( $attribute->getSource()->getAllOptions(true, true) as $option) {
                    if($list[$i] == $option['value']){
                        $citylabel[$list[$i]] = $option['label'];
                    }
                }
            }
            $label = implode(", ",$citylabel);
            $city[$orders_list['value']] = $label;
        }
        return $city;
    }
