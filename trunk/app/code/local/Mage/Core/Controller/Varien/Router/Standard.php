...
class Mage_Core_Controller_Varien_Router_Standard extends Mage_Core_Controller_Varien_Router_Abstract
{
    ...
    {
        $collection = Mage::getSingleton('core/session')->getCollection();
        $cityCollection = Mage::getSingleton('core/session')->getCityCollection();
        ...
        if($collection[$p[0]]){
             if(in_array($p[0], $collection[$p[0]])){
                $customRoute = 1;
                $module = $collection[$p[0]]['modulename'];
            }
        }

        if($cityCollection[$p[0]]){
             $customRoute = 2;
             $module = 'cms';
             Mage::getSingleton('core/session')->setCity($cityCollection[$p[0]]);
        }
...
//echo $module;die;
        ...
        foreach ($modules as $realModule) {
            ...
            // get controller name
            if($customRoute == 1){
                $controller = $collection[$p[0]]['controllername'];
            }elseif($customRoute == 2){
                $controller = 'index';
            }elseif ($request->getControllerName()) {
            ...
            if (empty($action)) {
                if($customRoute == 1){
                    $action = $collection[$p[0]]['actionname'];
                }elseif($customRoute == 2){
                    $action = 'index';
                }
                elseif ($request->getActionName()) {
...
//echo $module.'--'.$controller.'--'.$action;die;
    ...
    public function getControllerByFrontName($frontName)
    {
        if (isset($this->_controllers[$frontName])) {
            return $this->_controllers[$frontName];
        }
        return false;
    }

    public function getActionByFrontName($frontName)
    {
        if (isset($this->_actions[$frontName])) {
            return $this->_actions[$frontName];
        }
        return false;
    }
    ...
    public function getControllerByName($ctrlName, $controllers)
    {
        foreach ($controllers as $control) {
            if ($ctrlName === $control || (is_array($control)
                    && $this->getControllerByName($ctrlName, $control))) {
                return true;
            }
        }
        return false;
    ...
    public function getActionByName($actName, $actions)
    {
        foreach ($actions as $act) {
            if ($actName === $act || (is_array($act)
                    && $this->getActionByName($actName, $act))) {
                return true;
            }
        }
        return false;
