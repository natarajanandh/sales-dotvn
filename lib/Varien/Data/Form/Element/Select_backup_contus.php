...
class Varien_Data_Form_Element_Select extends Varien_Data_Form_Element_Abstract
{
    ...
    public function getElementHtml()
    {
        ...
        /* contus */ 
        if($this->getId() != 'deal_status'){
        return $html;}else {
              $html = '<label for="'.$this->getHtmlId() . $idSuffix . '">'.$this->_optionToHtml($option, $value, $dealstatus = true)
                . ( $this->getRequired() ? ' <span class="required">*</span>' : '' ).'</label>'."\n";
        }
    }

    protected function _optionToHtml($option, $selected, $dealstatus = false)
    {
        if (is_array($option['value'])) {
            ...
            if (in_array($option['value'], $selected)) { /* contus */
                if($this->getId() == 'deal_status'){$deal_staus = $option['label']; echo "<strong>".$deal_staus."</strong>";}
